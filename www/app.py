...
def init_jinja2(app, **kw):
    ...
    print('set jinja2 template path: %s' % path)
...
async def response_factory(app, handler):
    async def response(request):
        ...
        if isinstance(r, dict):
            template = r.get('__template__') # l获取网页资源名 模板
            if template is None:
                resp = web.Response(
...
async def init(loop):
    await orm.create_pool(loop=loop, host='localhost', port=3306, user='root', password='123456', db='python')
